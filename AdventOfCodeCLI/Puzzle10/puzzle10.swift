...
struct Puzzle: CustomStringConvertible {
    ...
    mutating func includeExclude() {
        ...
        for (y, row) in puzzle.enumerated() {
            ...
            for (x, tile) in row.enumerated() {
                ...
                if puzzle[y][x] == .empty {
                    puzzle[y][x] = (included && toggles % 2 == 1) ? .included : .empty
                }
            }
        }
        ...
        for x in 0..<puzzle[0].count {
            ...
            for y in 0..<puzzle.count {
                let tile = tile(at: Pos(x: x, y: y))
                if tile != .empty && tile != .start && tile != .included {
                    included.toggle()
                    toggles += 1
                }
                if puzzle[y][x] == .empty {
    ...
    mutating func excludeLeft(at pos: Pos) {
        switch tile(at: pos) {
        case .horizontal:
            setTypeIfEmpty(at: pos.upLeft, to: .excluded)
            setTypeIfEmpty(at: pos.up, to: .excluded)
            setTypeIfEmpty(at: pos.upRight, to: .excluded)
        case .vertical:
            break
        case .northEast:
            break
    ...
    mutating func setTypeIfEmpty(at pos: Pos, to newTile: Tile) {
        let tile = self.tile(at: pos)
        if(tile == .empty) {
            self.setType(at: pos, to: newTile)
        }
    }
