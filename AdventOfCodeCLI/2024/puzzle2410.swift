...
func puzzle2410() {
    ...
    var peakPositions: Set<Pos> = []


    func findNext(score: inout Int, height: Int, fromX x: Int, fromY y: Int, inDirection: Direction?) {
        if grid[y][x] != height {
            return
        }

        // print("\(inDirection?.direction() ?? "start") with height: \(height)")
        ...
        if x > 0 && inDirection != .right {
            findNext(score: &score, height: height + 1, fromX: x - 1, fromY: y, inDirection: .left)
        }
        if y > 0 && inDirection != .down {
            findNext(score: &score, height: height + 1, fromX: x, fromY: y - 1, inDirection: .up)
        }
        if x + 1 < grid[0].count && inDirection != .left {
            findNext(score: &score, height: height + 1, fromX: x + 1, fromY: y, inDirection: .right)
        }
        if y + 1 < grid.count && inDirection != .up {
            findNext(score: &score, height: height + 1, fromX: x, fromY: y + 1, inDirection: .down)
        }
    }
